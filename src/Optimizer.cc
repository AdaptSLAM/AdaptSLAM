...
/////////////////////////////////////////CommSLAM////////////////////////////////
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, Uncertainty *pUncertainty, bool bIsLoopClosing, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    */
    ...
    Eigen::MatrixXf Mtx;
    vector<KeyFrame*> vpKFsPre = pMap->GetAllKeyFrames();
    int SIZE_=vpKFsPre.size();
    vpMP = pMap->GetAllMapPoints(); 
    int Bandwidth=SIZE_/8+1;
    Eigen::MatrixXd LaplacianMatrix(SIZE_,SIZE_);

    int KeepNum=1;
    Eigen::MatrixXf MtxOld[KeepNum];
    Eigen::MatrixXf MtxOldInverse;
    Eigen::MatrixXf MtxTemp;
    

    vector<vector<int>> KeyFrameIdx;
    
    vector<vector<int>> KeyFrameIdxOld;
    
    vector<int> tempKeyFrameIdx;
    list<float> max_val;
    list<int> max_pos;
    
    list<int>::iterator apos;
    
    vector< vector<int> >::iterator aKeyFrameIdx;
    vector< vector<int> >::iterator aIsNotCandidate;
    float val;
    float detMtx;
    
    vector<vector<int>> IsNotCandidate;
    vector<vector<int>> IsNotCandidateOld;
    
    vector<int> tempIsNotCandidate;

    Eigen::VectorXf A12;
    vector<Eigen::MatrixXf> vShermanMatrix;
    int KeepNumCurrent;
    
    vector<KeyFrame*>::iterator it;
    cerr<<"vpKFsPre.size()"<<vpKFsPre.size()<<"Bandwidth"<<Bandwidth<<endl;
    if (vpKFsPre.size()<=Bandwidth+1)
    {
        vpKFs=vpKFsPre;
    }
    else
    {
        ...
        int i=0;
        for(vector<KeyFrame*>::iterator lit=vpKFsPre.begin(),lend=vpKFsPre.end();lit!=lend;lit++)
        {
            int j=i-1;
            for(vector<KeyFrame*>::iterator lit2=lit, lend2=vpKFsPre.end();lit2!=lend2;lit2++)
            {
                j++;
                if (lit!=lit2)
                {
                    float temp=-((*lit)->GetWeight((*lit2)))*0.0002;
                    LaplacianMatrix(i, j)= temp;
                    LaplacianMatrix(j, i)= temp;
                }
            }
            i++;
        }
        ...
        for (i=0;i<KeepNum;i++)
        {
            IsNotCandidateOld.push_back(vect);
        }
    
        int minval=min(SIZE_-1,KeepNum);
        priority_queue<pair<float, int>> q;
        for (int i = 0; i < SIZE_-1; i++) 
        {
            q.push(pair<float, int>(-LaplacianMatrix(SIZE_-1,i), i));
        }
        for (int i = 1; i <= minval; i++) 
        {
            MtxOld[i-1].resize(1,1);
            (MtxOld[i-1])(0,0)=q.top().first; 
            tempKeyFrameIdx={q.top().second};
            KeyFrameIdxOld.push_back(tempKeyFrameIdx);
            IsNotCandidateOld[i-1][q.top().second]=1;
            q.pop();
        }
    
    
        KeepNumCurrent=minval;
    
        for (i=1;i<Bandwidth;i++)//Add one keyframe each time
        {

            MtxOldInverse.resize(i,i);
            MtxTemp.resize(i,i);
            A12.resize(i);
            for (int l=0;l<KeepNumCurrent;l++)
            {
                MtxOldInverse=MtxOld[l].inverse();
            
                for (int j=0;j<i;j++)
                {
                    //Sherman–Morrison formula
                    vShermanMatrix.push_back(MtxOldInverse.col(j)*MtxOldInverse.row(j));
                }
            
            
                for (int k=0;k<vpKFsPre.size();k++) //choose from keyframes
                {
                    if (IsNotCandidateOld[l][k]==1)
                        continue;

                    MtxTemp=MtxOldInverse;
                    val=log(MtxOld[l].determinant());
            
                    for (int j=0;j<i;j++)
                    {
                        /////////////////////[MtxOld,A12;A21,A22]
                        A12(j)=LaplacianMatrix(KeyFrameIdxOld[l][j],k);
                        if (-A12(j)>0.01)
                        {
                            val=val+log(1-MtxTemp(j,j)*A12(j));
                            MtxTemp=MtxTemp+(A12(j)*vShermanMatrix[j])/(1-A12(j)*MtxTemp(j,j));
                        }
                    }
            
            
                    val=val+log(-A12.sum()-LaplacianMatrix(SIZE_-1,k)-A12.transpose()*MtxTemp*A12);
                
                    if (max_val.empty())
                    {
                        max_val.push_back(val);
                        max_pos.push_back(k);
                    
                        tempKeyFrameIdx=KeyFrameIdxOld[l];
                        tempKeyFrameIdx.push_back(k);
                    
                        KeyFrameIdx.push_back(tempKeyFrameIdx);
                    
                    
                        tempIsNotCandidate=IsNotCandidateOld[l];
                        tempIsNotCandidate[k]=1;
                        IsNotCandidate.push_back(tempIsNotCandidate);
                    }
                    else if (max_val.size()<KeepNum)
                    {
                        apos=max_pos.begin();
                        aKeyFrameIdx=KeyFrameIdx.begin();
                        aIsNotCandidate=IsNotCandidate.begin();
                        int b=-1;
                        int tempsize=max_val.size();
                    
                        for (list<float>::iterator a=max_val.begin();a!=max_val.end();a++)
                        {
                            b++;
                            tempKeyFrameIdx=KeyFrameIdxOld[l];
                            tempKeyFrameIdx.push_back(k);
                            ...
                            if (val> *a)
                            {
                                max_val.insert(a,val);
                                advance(apos,b);
                                max_pos.insert(apos,k);

                                advance(aKeyFrameIdx,b);
                                KeyFrameIdx.insert(aKeyFrameIdx,tempKeyFrameIdx);
                                advance(aIsNotCandidate,b);
                                IsNotCandidate.insert(aIsNotCandidate,tempIsNotCandidate);
                                break;
                            }
                    ...
                    else if ((val>max_val.back()))
                    {
                        ...
                        aIsNotCandidate=IsNotCandidate.begin();
                        int b=-1;
                        for (list<float>::iterator a=max_val.begin(),aend=max_val.end();a!=aend;a++)
                        {
                            b++;
                            if (val>*a)
                            {
                        ...
                        max_val.pop_back();
                        max_pos.pop_back();
                        KeyFrameIdx.pop_back();
                        IsNotCandidate.pop_back();
                    }
                }
        
                vShermanMatrix.clear();
            }

            for (int l=0;l<max_val.size();l++)
            {
                MtxOld[l].resize(i+1,i+1);
                for (int j=0;j<=i;j++)
                {
                    for (int k=j;k<=i;k++)
                    {
                        if (j!=k)
                        {
                            (MtxOld[l])(j,k)=LaplacianMatrix(KeyFrameIdx[l][j],KeyFrameIdx[l][k]);
                            //cerr<<"KeyFrameIdx[l][j]"<<KeyFrameIdx[l][j]<<"KeyFrameIdx[l][i]"<<KeyFrameIdx[l][i]<<endl;
                            (MtxOld[l])(k,j)=(MtxOld[l])(j,k);
                        }
                        else
                        {
                            (MtxOld[l])(j,j)=-LaplacianMatrix(SIZE_-1,KeyFrameIdx[l][j]);
                        }
                    }
                }
            ...
            KeepNumCurrent=max_val.size();
            max_val.clear();
            max_pos.clear();
            
            KeyFrameIdxOld=KeyFrameIdx;
            IsNotCandidateOld=IsNotCandidate;
            KeyFrameIdx.clear();
            IsNotCandidate.clear();
        }
    
        tempKeyFrameIdx=KeyFrameIdxOld.front();
        for (int i=0;i<Bandwidth;i++)
        {
            it = vpKFsPre.begin();
            advance(it,tempKeyFrameIdx[i]);
            vpKFs.push_back(*it);
        }
        Mtx.resize(Bandwidth-1,Bandwidth-1);
        Mtx=MtxOld[0];
        }
        ...
        MtxOld.resize(1,1);
        MtxOld(0,0)=-LaplacianMatrix(0,1); 
        KeyFrameIdx[0]=1;
        ...
        for (i=1;i<Bandwidth;i++)//Add one keyframe each time
        {
            ...
            for (int k=0;k<vpKFsPre.size();k++) //choose from keyframes
            {
                if (IsNotCandidate[k]==1)
                    continue;

                MtxTemp=MtxOldInverse;
                detMtx=MtxOld.determinant();
            
                for (int j=0;j<i;j++)
                {
                    /////////////////////[MtxOld,A12;A21,A22]
                    A12(j)=LaplacianMatrix(KeyFrameIdx[j],k);
                    if (-A12(j)>0.01)
                    {
                        detMtx=detMtx*(1-MtxTemp(j,j)*A12(j));
                        MtxTemp=MtxTemp+(A12(j)*vShermanMatrix[j])/(1-A12(j)*MtxTemp(j,j));
                    }
                }
                val=log(detMtx)+log(-A12.sum()-LaplacianMatrix(0,k)-A12.transpose()*MtxTemp*A12);
                if (val>max_val)
                {
                    max_val=val;
                    max_pos=k;
            ...
            cerr<<"max_val"<<max_val<<"max_pos"<<max_pos<<endl;
            it = vpKFsPre.begin();
            advance(it, max_pos);
            vpKFs.push_back(*it);
            KeyFrameIdx[i]=max_pos;
            IsNotCandidate[max_pos]=1;
        
            Mtx.resize(i+1,i+1);
            Mtx.block(0,0,i,i)=MtxOld;
            Mtx(i,i)=-LaplacianMatrix(0,max_pos);
            for (int j=0;j<i;j++)
            {
                Mtx(j,i)=LaplacianMatrix(KeyFrameIdx[j],max_pos);
                Mtx(i,j)=Mtx(j,i);
                Mtx(j,j)=Mtx(j,j)-Mtx(j,i);
                Mtx(i,i)=Mtx(i,i)-Mtx(j,i);
            }
            MtxOld.resize(i+1,i+1);
            MtxOld=Mtx;
            vShermanMatrix.clear();
        }
    
    
    for(it = vpKFs.begin();it!=vpKFs.end();it++)
    {
        cerr<<"mnId "<<(*it)->mnId<<endl;
    }
    */
    ...
    int SIZE_=vpKFsPre.size();
    vpMP = pMap->GetAllMapPoints(); 
    int nUsrsPerIterations=5;
    int nIt=floor(SIZE_/nUsrsPerIterations);
    float max_val;
    int max_pos;
    bool bIsCommSlam=1;
    if (nIt<1)
    ...
    {
        for (int i=0; i<nUsrsPerIterations;i++)
        {
            vpKFs.push_back(vpKFsPre[i]);
        }
        for (int i=1; i<nIt; i++)
        {
            if (bIsCommSlam)
            {
                max_val=-1000;
                max_pos=0;
                for (int j=i*nUsrsPerIterations;j<(i+1)*nUsrsPerIterations;j++)
                {
                    float val=pUncertainty->UpdateUncertainty(vpKFs,vpKFsPre[j]);
                    if (val>max_val)
                    {
                        max_val=val;
                        max_pos=j;
                ...
                cerr<<"vpKFs.size()"<<vpKFs.size()<<"max_val"<<max_val<<endl;
                //max_pos=rand()%nUsrsPerIterations+i*nUsrsPerIterations;
                vpKFs.push_back(vpKFsPre[max_pos]);
            }
            else
            {
                max_pos=rand()%nUsrsPerIterations+i*nUsrsPerIterations;
        ...
        if (nUsrsPerIterations*nIt<SIZE_)
        {
            if (bIsCommSlam)
            {
                max_val=-1000;
                max_pos=0;
                for (int i=nUsrsPerIterations*nIt;i<SIZE_;i++)
                {
                    float val=pUncertainty->UpdateUncertainty(vpKFs,vpKFsPre[i]);
                    if (val>max_val)
                    {
                        max_val=val;
                        max_pos=i;
            ...
            {
                max_pos= rand()%(SIZE_-nUsrsPerIterations*nIt)+nUsrsPerIterations*nIt;

            }
            //max_pos= rand()%(SIZE_-nUsrsPerIterations*nIter)+nUsrsPerIterations*nIter;
            vpKFs.push_back(vpKFsPre[max_pos]);
        }
    }
        */
    ...
    if (bIsLoopClosing)
    {
        pUncertainty->GlobalMapUncertainty(pMap);
    }
    */
}
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->isBad() || pKF->mnId>maxKFid)
                continue;
            ///////////////////////////////////////CommSLAM////////////////////////
            if (find(vpKFs.begin(), vpKFs.end(), pKF) == vpKFs.end())
                continue;
...
///////////////////////////////////////////CommSLAM//////////////////////////////////////////////
void Optimizer::FullInertialBA(Map *pMap, Uncertainty *pUncertainty, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    ...
    /////////////////////////////////CommSLAM////////////////////////////////
    pUncertainty->GlobalInertialUncertainty(pMap);
}
...
///////////////////////////////////CommSLAM/////////////////////////////////////////////////
void Optimizer::EdgeBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, Uncertainty *pUncertainty, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    */
    ...
    vector<KeyFrame*> vLocalKeyFrames=pMap->GetAllKeyFrames();
    list<KeyFrame*> lLocalKeyFrames(vLocalKeyFrames.begin()+vLocalKeyFrames.size()/2,vLocalKeyFrames.end());

    
    vector<MapPoint*> vLocalMapPoints=pMap->GetAllMapPoints();
    list<MapPoint*> lLocalMapPoints(vLocalMapPoints.begin(),vLocalMapPoints.end());
    ...
    if (lLocalKeyFrames.size()>AnchorSize)
    {
        for (int i=0;i<AnchorSize;i++)
        {
            tempKF=lLocalKeyFrames.front();
            lLocalKeyFrames.pop_front();
            lFixedCameras.push_back(tempKF);
        }
    ...
    for(it = lLocalKeyFrames.begin();it!=lLocalKeyFrames.end();it++)
    {
        (*it)->mnEdgeForKF = pKF->mnId;
    }
    
    
    for(it = lFixedCameras.begin();it!=lFixedCameras.end();it++)
    {
        (*it)->mnEdgeFixedForKF = pKF->mnId;
    }
    num_fixedKF = 0;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            ///////////////////////////////////CommSLAM/////////////////////////////////////////////////
             ...
             if(pKFi->mnEdgeForKF!=pKF->mnId && pKFi->mnEdgeFixedForKF!=pKF->mnId)
                continue;
...
///////////////////////////////////CommSLAM/////////////////////////////////////////////////
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, Uncertainty *pUncertainty, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    // Local KeyFrames: First Breath Search from Current Keyframe
    /*
    list<KeyFrame*> lLocalKeyFrames;
    ...
    */
    ...
    list<KeyFrame*> lLocalKeyFramesPre;

    list<KeyFrame*> lFixedCamerasPre;
    
    lLocalKeyFrames.push_back(pKF);
    
    lLocalKeyFramesPre.push_back(pKF);
    ...
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        KeyFrame* pKFi = vNeighKFs[i];
        if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            lLocalKeyFramesPre.push_back(pKFi);
    }

    int SIZE_=lLocalKeyFramesPre.size();

    list<KeyFrame*>::iterator it;
    
    int LocalMapSize=30;
    int LocalAnchorSize=5;
    Eigen::MatrixXf Mtx;
    //vector<Eigen::MatrixXf> MtxOld;
    ...
    int i=0;
    for(list<KeyFrame*>::iterator lit=lLocalKeyFramesPre.begin(),lend=lLocalKeyFramesPre.end();lit!=lend;lit++)
    {
        int j=i-1;
        for(list<KeyFrame*>::iterator lit2=lit, lend2=lLocalKeyFramesPre.end();lit2!=lend2;lit2++)
        {
            j++;
            if (lit!=lit2)
            {
                float temp=-((*lit)->GetWeight((*lit2)));
                LaplacianMatrix(i, j)= temp;
                LaplacianMatrix(j, i)= temp;
            }
        }
    ...
    if (lLocalKeyFramesPre.size()<=LocalMapSize+1)
    {
        lLocalKeyFrames=lLocalKeyFramesPre;
        int PreSize=lLocalKeyFramesPre.size();
        Mtx.resize(PreSize,PreSize);
    ...
    {
        MtxOld[i-1].resize(1,1);
        (MtxOld[i-1])(0,0)=-LaplacianMatrix(0,i); 
        tempKeyFrameIdx={i};
        KeyFrameIdxOld.push_back(tempKeyFrameIdx);
        IsNotCandidateOld[i-1][i]=1;
    }
    
    KeepNumCurrent=minval;
    
    for (i=1;i<LocalMapSize;i++)//Add one keyframe each time
    {
        ...
        for (int l=0;l<KeepNumCurrent;l++)
        {
            ...
            for (int k=0;k<lLocalKeyFramesPre.size();k++) //choose from keyframes
            {
                if (IsNotCandidateOld[l][k]==1)
                    continue;

                MtxTemp=MtxOldInverse;
                detMtx=MtxOld[l].determinant();
            ...
            //it = lLocalKeyFramesPre.begin();
            //advance(it, max_pos);
            //lLocalKeyFrames.push_back(*it);
            /*
            Mtx.resize(i+1,i+1);
            Mtx.block(0,0,i,i)=MtxOld[l];
            Mtx(i,i)=-LaplacianMatrix(0,max_pos);
            for (int j=0;j<i;j++)
            {
                Mtx(j,i)=LaplacianMatrix(KeyFrameIdx[j],max_pos);
        ...
        for (int l=0;l<max_val.size();l++)
        {
            ...
            for (int j=0;j<=i;j++)
            {
                for (int k=j;k<=i;k++)
                {
                    ...
                    {
                        (MtxOld[l])(j,j)=-LaplacianMatrix(0,KeyFrameIdx[l][j]);
                    }
                }
            }
    ...
    Mtx.resize(LocalMapSize-1,LocalMapSize-1);
    Mtx=MtxOld[0];
    }
    }
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            ////////////////CommSLAM///////////////////
            if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAIsAddedBefore!=pKF->mnId)
            {                
                ...
                {
                    ////////////////CommSLAM///////////////////
                    lFixedCamerasPre.push_back(pKFi);
                    pKFi->mnBAIsAddedBefore=pKF->mnId;
                }
                    /*
                    int j=0;
                    for (it=lLocalKeyFramesPre.begin();it!=lLocalKeyFramesPre.end();it++)
                    {
                        j++;
                        if (j>nUsrsPerIterations+5)
                            break;
                        if ((*it)->mnId==pKFi->mnId)
                        {
                            lFixedCameras.push_back(pKFi);
                            pKFi->mnBAFixedForKF=pKF->mnId;
                            break;
    ...
    if (lFixedCamerasPre.size()<=LocalAnchorSize)
    {
        lFixedCameras=lFixedCamerasPre;
    }
    else{
    ...
    MtxOldInverse.resize(lLocalKeyFrames.size(),lLocalKeyFrames.size());
    
    MtxTemp.resize(lLocalKeyFrames.size(),lLocalKeyFrames.size());

    for (int i=0;i<LocalAnchorSize;i++)//Add one keyframe each time
    {
        
        MtxOldInverse=Mtx.inverse();
        ...
        pKFmax=lFixedCamerasPre.front();
        max_val_fixed=-10000;
        for (list<KeyFrame*>::iterator lit=lFixedCamerasPre.begin();lit!=lFixedCamerasPre.end();lit++)
        {
            if((*lit)->mnBAFixedForKF==pKF->mnId)
                continue;
            detMtx=Mtx.determinant();
            MtxTemp=MtxOldInverse;
            
            int j=-2;
            for(list<KeyFrame*>::iterator lit2 = lLocalKeyFrames.begin();lit2!=lLocalKeyFrames.end();lit2++)
            {
                j++;
                if (j>=0)
                {float temp=-((*lit)->GetWeight((*lit2)));
                ...
                if (-temp>0.01)
                {
                    detMtx=detMtx*(1-MtxTemp(j,j)*temp);
                    MtxTemp=MtxTemp+(temp*vShermanMatrix[j])/(1-temp*MtxTemp(j,j));
                }}
                        
            }
            if (detMtx>max_val_fixed)
            {
                max_val_fixed=detMtx;
                pKFmax=*lit;
            }
                    
        }
        
        lFixedCameras.push_back(pKFmax);
        //cerr<<"FixedmnId"<<pKFmax->mnId<<endl;
        pKFmax->mnBAFixedForKF=pKF->mnId;
        ...
        for(list<KeyFrame*>::iterator lit2 = lLocalKeyFrames.begin();lit2!=lLocalKeyFrames.end();lit2++)
        {
            ...
            if(j>=0)
            {
            Mtx(j,j)=Mtx(j,j)+(pKFmax)->GetWeight(*lit2);
            }
        }
        ...
        vShermanMatrix.clear();
            ...
            cerr<<"lLocalKeyFrames.size()"<<lLocalKeyFrames.size()<<"lFixedCameras.size()"<<lFixedCameras.size()<<endl;
    ...
    pUncertainty->LocalMapUncertainty(lLocalKeyFrames,pMap);
}
...
////////////////////////////CommSLAM////////////////////////////////////////////
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, Uncertainty *pUncertainty, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    /*
    Map* pCurrentMap = pKF->GetMap();
    ...
    vector<KeyFrame*> vpOptimizableKFs;//This is for inertial edges
    const vector<KeyFrame*> vpNeighsKFs = pKF->GetVectorCovisibleKeyFrames();
    list<KeyFrame*> lpOptVisKFs;//This is for covisibility edges
    ...
    if(vpOptimizableKFs.back()->mPrevKF)
    {
        ...
        vpOptimizableKFs.back()->mPrevKF->mnBAFixedForKF=pKF->mnId;
           lFixedCamerasPre.push_back(vpOptimizableKFs.back()->mPrevKF);
    }
    else
    {
        ...
        lFixedKeyFrames.push_back(vpOptimizableKFs.back());
            lFixedCamerasPre.push_back(vpOptimizableKFs.back());
    ...
    */
    ...
    ////////////////CommSLAM, this is the local map size//////////////////////
    int maxOpt=10;
    /////////////CommSLAM////////////////
    int InertialSize=0;
    
    
    int InertialEdgeWeight=0.1;
    float weight=0.002;
    ...
    if(bLarge)
    {
        ...
        ///////////////CommSLAM///////////////////////
    int Nd = std::min((int)pCurrentMap->KeyFramesInMap()-2,maxOpt);
    ...
    vector<KeyFrame*> vpOptimizableKFsPre;

    list<KeyFrame*> lFixedCamerasPre;
    

    
    vpOptimizableKFsPre.push_back(pKF);
    
    
    for(int i=1; i<Nd; i++)
    {
        if(vpOptimizableKFsPre.back()->mPrevKF)
        {
            InertialSize++;
            vpOptimizableKFsPre.push_back(vpOptimizableKFsPre.back()->mPrevKF);
            vpOptimizableKFsPre.back()->mnBALocalForKF = pKF->mnId;
        }
        else
            break;
    }
    ...
    if(vpOptimizableKFsPre.back()->mPrevKF)
    {
        ...
        lFixedCamerasPre.push_back(vpOptimizableKFsPre.back()->mPrevKF);
        lFixedKeyFrames.push_back(vpOptimizableKFsPre.back()->mPrevKF);
        
        vpOptimizableKFsPre.back()->mPrevKF->mnBAFixedForKF=pKF->mnId;
    }
    else
    {
        vpOptimizableKFsPre.back()->mnBALocalForKF=0;
        vpOptimizableKFsPre.back()->mnBAFixedForKF=pKF->mnId;
        lFixedCamerasPre.push_back(vpOptimizableKFsPre.back());
        lFixedKeyFrames.push_back(vpOptimizableKFsPre.back());
        
        vpOptimizableKFsPre.pop_back();
        InertialSize--;
    }
    ...
    vpOptimizableKFs=vpOptimizableKFsPre;
    const vector<KeyFrame*> vNeighKFs = pKF->GetVectorCovisibleKeyFrames();
    cerr<<"vNeighKFs.size()"<<vNeighKFs.size()<<endl;
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        KeyFrame* pKFi = vNeighKFs[i];
        if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
        {
            if ((std::find(vpOptimizableKFsPre.begin(), vpOptimizableKFsPre.end(), pKFi) == vpOptimizableKFsPre.end())&&(pKFi->mnBAFixedForKF!=pKF->mnId))
            {
                vpOptimizableKFsPre.push_back(pKFi);
            }
        }
    }
    }
    
    int SIZE_=vpOptimizableKFsPre.size();

    list<KeyFrame*>::iterator it;
    vector<KeyFrame*>::iterator vecit;
    ...
    int i=0;
    for(vector<KeyFrame*>::iterator lit=vpOptimizableKFsPre.begin(),lend=vpOptimizableKFsPre.end();lit!=lend;lit++)
    {
        if (i<InertialSize)
        {
            LaplacianMatrix(i, i+1)-= InertialEdgeWeight;
            LaplacianMatrix(i+1, i)-= InertialEdgeWeight;
    
        }
        
        int j=i-1;
        for(vector<KeyFrame*>::iterator lit2=lit, lend2=vpOptimizableKFsPre.end();lit2!=lend2;lit2++)
        {
            j++;
            if (lit!=lit2)
            {
                float temp=-((*lit)->GetWeight((*lit2)))*weight;
                LaplacianMatrix(i, j)+= temp;
                LaplacianMatrix(j, i)+= temp;
            }
        }
    ...
    //cerr<<"LaplacianMatrix"<<LaplacianMatrix<<endl;
    if (vpOptimizableKFsPre.size()<=maxOpt+1)
    {
     
        vpOptimizableKFs=vpOptimizableKFsPre;
        int PreSize=vpOptimizableKFsPre.size();
        Mtx.resize(PreSize,PreSize);
        for (int i=0;i<PreSize;i++)
        {
            for (int j=0;j<PreSize;j++)
        ...
        Mtx=Mtx.block(1,1,PreSize-1,PreSize-1);
        //cerr<<log(Mtx.determinant())<<"Mtx.determinant()"<<Mtx.size()<<endl;
    }
    else{
    ...
    for (i=1;i<maxOpt;i++)//Add one keyframe each time
    {
        ...
        for (int l=0;l<KeepNumCurrent;l++)
        {
            ...
            for (int k=0;k<vpOptimizableKFsPre.size();k++) //choose from keyframes
            {
                if (IsNotCandidateOld[l][k]==1)
                    continue;
                ...
                val=val+log(-A12.sum()-LaplacianMatrix(0,k)-A12.transpose()*MtxTemp*A12);
                //cerr<<"localMap"<<val;
                if (max_val.empty())
                {
                    max_val.push_back(val);
    ...
    {
        vecit = vpOptimizableKFsPre.begin();
        advance(vecit,tempKeyFrameIdx[i]);
        vpOptimizableKFs.push_back(*vecit);
        (*vecit)->mnBALocalForKF=pKF->mnId;
    }
    Mtx.resize(maxOpt-1,maxOpt-1);
    Mtx=MtxOld[0];
    }
    }
    ...
    for(vecit = vpOptimizableKFs.begin();vecit!=vpOptimizableKFs.end();vecit++)
    {
        //cerr<<"mnId "<<(*vecit)->mnId<<endl;
        (*vecit)->mnBALocalForKF = pKF->mnId;
    }
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            //if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAFixedForKF!=pKF->mnId)
                  ////////////////CommSLAM///////////////////
            if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAIsAddedBefore!=pKF->mnId && pKFi->mnBAFixedForKF!=pKF->mnId)
            {
                //pKFi->mnBAFixedForKF=pKF->mnId;
                /////////////////CommSLAM///////////////
                pKFi->mnBAIsAddedBefore=pKF->mnId;
    ...
    //     cerr<<"lFixedCamerasPre"<<lFixedCamerasInePre.size()<<endl;
   // for(it = lFixedCamerasPre.begin();it!=lFixedCamerasPre.end();it++)
  //  {
  //      cerr<<(*it)->mnId<<" "<<endl;
  //  }
    ...
    ////////////////CommSLAM/////////////////////////////
     //cerr<<log(Mtx.determinant())<<"Mtx!!!!!!!!!!!!!!!!!!"<<Mtx<<endl;
    if (lFixedCamerasPre.size()<=maxFixKF)
    {
        lFixedKeyFrames=lFixedCamerasPre;
    }
    else{
    ...
    MtxOldInverse.resize(vpOptimizableKFs.size(),vpOptimizableKFs.size());
    
    MtxTemp.resize(vpOptimizableKFs.size(),vpOptimizableKFs.size());

    for (int i=0;i<maxFixKF;i++)//Add one keyframe each time
    {
        ...
        pKFmax=lFixedCamerasPre.front();
        max_val_fixed=-100000000;
        for (list<KeyFrame*>::iterator lit=lFixedCamerasPre.begin();lit!=lFixedCamerasPre.end();lit++)
        {
            if((*lit)->mnBAFixedForKF==pKF->mnId)
                continue;
            val=log(Mtx.determinant());
            MtxTemp=MtxOldInverse;
            
            int j=-2;
            for(vector<KeyFrame*>::iterator lit2 = vpOptimizableKFs.begin();lit2!=vpOptimizableKFs.end();lit2++)
            {
                j++;
                if (j>=0)
                {float temp=-((*lit)->GetWeight((*lit2)))*weight;
                ...
                if (-temp>weight)
                {
                    val=val+log(1-MtxTemp(j,j)*temp);
                    MtxTemp=MtxTemp+(temp*vShermanMatrix[j])/(1-temp*MtxTemp(j,j));
                    //cerr<<"MtxTemp(j,j)"<<MtxTemp(j,j)<<"temp"<<temp;
                }}
                        
            }
            //if (pKFmax!=lFixedCamerasPre.front())
                cerr<<"val"<<val;
            if (val>max_val_fixed)
            {
                max_val_fixed=val;
                pKFmax=*lit;
            }
        ...
        if(pKFmax->mnBAFixedForKF==pKF->mnId)
            continue;
        
        lFixedKeyFrames.push_back(pKFmax);
        ...
        for(vector<KeyFrame*>::iterator lit2 = vpOptimizableKFs.begin();lit2!=vpOptimizableKFs.end();lit2++)
        {
            ...
            if(j>=0)
            {
            Mtx(j,j)=Mtx(j,j)+(pKFmax)->GetWeight(*lit2)*weight;
            }
        }
    ...
    for(it = lFixedKeyFrames.begin();it!=lFixedKeyFrames.end();it++)
    {
        (*it)->mnBAFixedForKF=pKF->mnId;
        cerr<<"FixedmnId"<<(*it)->mnId<<" "<<(*it)->mnBAFixedForKF<<endl;
    }
    }
    ...
    for(int i=0;i<N;i++)
    {
        ...
        {
            ///////////////////CommSLAM///////////////////////////
            if (std::find(vpOptimizableKFs.begin(), vpOptimizableKFs.end(), pKFi->mPrevKF) == vpOptimizableKFs.end())
                
                ////////////////////////CommSLAM////////////
                continue;
                //break;
            //cerr<<"TEST"<<pKFi->mnId<<" "<<pKFi->mPrevKF->mnId<<endl;
            pKFi->mpImuPreintegrated->SetNewBias(pKFi->mPrevKF->GetImuBias());
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pKFi->mPrevKF->mnId);
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+3*(pKFi->mPrevKF->mnId)+1);
    ...
    pMap->IncreaseChangeIndex();
    /////////////////////////////////CommSLAM////////////////////////////////
    //pUncertainty->LocalInertialUncertainty(vpOptimizableKFs);
}
...
////////////////////////CommSLAM//////////////////////////////////
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, Uncertainty* pUncertainty, bool *pbStopFlag)
{
    ...
    ///////////////////////////////CommSLAM//////////////////////
    cerr<<"Merge visual!!!!!!!!!"<<endl;
    pUncertainty->MergeVisualUncertainty(pMainKF, vpAdjustKF, vpFixedKF);
