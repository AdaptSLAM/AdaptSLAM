...
{
///////////////////////////////CommSLAM//////////////////////////////////////////////////
LoopClosing::LoopClosing(Atlas *pAtlas, Uncertainty *pUncertainty, KeyFrameDatabase *pDB, ORBVocabulary *pVoc,  const bool bFixScale, const bool bActiveLC):
    mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas), mpUncertainty(pUncertainty), mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false), mbFinishedGBA(true), mbStopGBA(false), mpThreadGBA(NULL), mbFixScale(bFixScale), mnFullBAIdx(0), mnLoopNumCoincidences(0), mnMergeNumCoincidences(0), mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        /*
        //////////////////////////////CommSLAM/////////////////////////////////////////
        if(CheckNewKeyFrames()&&(mpCurrentKF->GetMap()->isImuInitialized())&&(mpCurrentKF->mnId))
        {
        cerr<<"test1"<<"mpCurrentKF->GetMap()->isImuInitialized()"<<mpCurrentKF->GetMap()->isImuInitialized()<<"mpCurrentKF->mnId"<<mpCurrentKF->mnId<<endl;;
        mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment,this, mpCurrentKF->GetMap(), mpCurrentKF->mnId);
        cerr<<"test2"<<endl;
        }
        */
...
void LoopClosing::MergeLocal()
{
    ...
    {
        ///////////////////////////////////CommSLAM////////////////////////////////////////
        Optimizer::LocalBundleAdjustment(mpCurrentKF, vpLocalCurrentWindowKFs, vpMergeConnectedKFs,mpUncertainty, &bStop);
    }
    ...
    if(vpCurrentMapKFs.size() == 0){}
        ...
        ///////////////CommSLAM////////////
        /*
        for (int i=0;i<vpCurrentMapKFs.size()*7/8;i++)
        {
            vpCurrentMapKFs.erase(vpCurrentMapKFs.begin());
            //vpCurrentMapKFs.pop_back();
        }
        */
        vector<KeyFrame*> vpKFs;
        
        Eigen::MatrixXf Mtx;
        vector<KeyFrame*> vpKFsPre = pCurrentMap->GetAllKeyFrames();
        int SIZE_=vpKFsPre.size();
        //vpMP = pCurrentMap->GetAllMapPoints(); 
        ...
        for (i=1;i<Bandwidth;i++)//Add one keyframe each time
        {
            ...
            for (int l=0;l<KeepNumCurrent;l++)
            {
                ...
                for (int k=0;k<vpKFsPre.size();k++) //choose from keyframes
                {
                    ...
                    val=log(detMtx)+log(-A12.sum()-LaplacianMatrix(SIZE_-1,k)-A12.transpose()*MtxTemp*A12);
...
{  
    ...
    if(!bImuInit)
        ////////////////////////////////CommSLAM/////////////////////////////////////////
        //Optimizer::GlobalBundleAdjustemnt(pActiveMap,mpUncertainty,true,10,&mbStopGBA,nLoopKF,false);
        Optimizer::GlobalBundleAdjustemnt(pActiveMap,mpUncertainty,true,10,&mbStopGBA,nLoopKF,false);
    else
        ////////////////////////////////CommSLAM/////////////////////////////////////////
        Optimizer::FullInertialBA(pActiveMap,mpUncertainty,7,false,nLoopKF,&mbStopGBA);
