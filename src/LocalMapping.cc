...
{
//////////////CommSLAM////////////////////////////
LocalMapping::LocalMapping(System* pSys, Atlas *pAtlas, Uncertainty *pUncertainty, const float bMonocular, bool bInertial, const string &_strSeqName):
    mpSystem(pSys), mbMonocular(bMonocular), mbInertial(bInertial), mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas), mpUncertainty(pUncertainty), bInitializing(false),
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    else
                    {
                        ...
                        if (radnum==1)
                        {
                        //mpLoopCloser->RunGlobalBundleAdjustment(mpCurrentKeyFrame->GetMap(),mpCurrentKeyFrame->mnId);
                        }
                        //Optimizer::EdgeBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA, mpCurrentKeyFrame->GetMap(),mpUncertainty, num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA);
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    cerr<<"mpCurrentKeyFrame"<<mpCurrentKeyFrame->mnId<<endl;
